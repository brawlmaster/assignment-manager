...
function render(){
	...
	for (const t of sorted){
		...
		if (t.url) {
			const link=document.createElement('a'); link.href=t.url; link.target='_blank'; link.rel='noopener noreferrer'; link.textContent=' • Link'; link.style.marginLeft='6px';
...
function openDialog(defaults=null){ form.reset(); document.getElementById('dialogTitle').textContent = defaults?'Edit Task':'New Task'; const d=defaults ?? { title:'', due: nowMs()+ONE_DAY_MS, importance:5, notes:'', url:'', recurrence:'none', tags:[] }; titleInput.value=d.title; dueInput.value=toInputDateTime(d.due); importanceInput.value=d.importance; if (importanceValueEl) importanceValueEl.textContent=String(d.importance); recurrenceInput.value=d.recurrence; notesInput.value=d.notes||''; if (urlInput) urlInput.value=d.url||''; if (tagsInput) tagsInput.value=(d.tags||[]).join(', '); dialog.showModal(); }
...
form.addEventListener('submit', async (e)=>{ if (!e.submitter || e.submitter.id!=='saveTaskButton'){ e.preventDefault(); return; } e.preventDefault(); const title=titleInput.value.trim(); const due=new Date(dueInput.value).getTime(); const importance=Number(importanceInput.value); const recurrence=recurrenceInput.value; const notes=notesInput.value.trim(); const url=(urlInput?.value||'').trim(); const tags=(tagsInput?.value||'').split(',').map(s=>s.trim()).filter(Boolean); if(!title||!Number.isFinite(due)) return; const task={ id:uid(), title, due, importance, notes, url, recurrence, tags, completed:false, createdAt: nowMs() }; await saveTask(task); tasks.push(task); tasks.sort((a,b)=>a.due-b.due); render(); dialog.close(); });
...
class MusicPlayer {
	...
	init() {
		...
		if (this.skipForward) this.skipForward.addEventListener('click', () => this.skipForwardTime());
		if (this.skipBackward) this.skipBackward.addEventListener('click', () => this.skipBackwardTime());
		if (this.volumeSlider) this.volumeSlider.addEventListener('input', (e) => this.setVolume(e.target.value));
		if (this.minimizeBtn) this.minimizeBtn.addEventListener('click', () => this.toggleMinimize());
		this.audio.addEventListener('play', () => this.onPlay());
		this.audio.addEventListener('pause', () => this.onPause());
		this.audio.addEventListener('ended', () => this.onEnded());
	}
	togglePlay() { this.isPlaying ? this.pause() : this.play(); }
	play() { this.audio.play().catch(()=> this.showToast('Click to enable audio playback')); }
	pause() { this.audio.pause(); }
	skipForwardTime() { this.audio.currentTime = Math.min(this.audio.currentTime + 10, this.audio.duration); this.showToast('Skipped forward 10s'); }
	skipBackwardTime() { this.audio.currentTime = Math.max(this.audio.currentTime - 10, 0); this.showToast('Skipped backward 10s'); }
	setVolume(value) { this.audio.volume = value / 100; }
	onPlay() { this.isPlaying = true; this.vinylRecord.classList.add('playing'); this.vinylArm.classList.add('playing'); this.controlPlayIcon.style.display = 'none'; this.controlPauseIcon.style.display = 'block'; }
	onPause() { this.isPlaying = false; this.vinylRecord.classList.remove('playing'); this.vinylArm.classList.remove('playing'); this.controlPlayIcon.style.display = 'block'; this.controlPauseIcon.style.display = 'none'; }
	onEnded() {}
	showToast(message) {
		const toast = document.createElement('div');
		toast.style.cssText = 'position:fixed;bottom:100px;right:20px;background:rgba(59,130,246,.9);color:#fff;padding:12px 16px;border-radius:8px;z-index:1000;';
